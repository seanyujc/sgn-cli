...
export function createFile(tplRoot: string, pageName: string) {
    ...
    fs.readdir(_moduleTplRoot, (err, files) => {
        ...
        files.forEach((fileName, index, array) => {
            ...
            let _pathName = path.join(_moduleTplRoot, fileName)
            fs.readFile(_pathName, (err, data) => {
                let content = commons.replaceKeyword(data.toString('utf8'), pageName)
                let dir = path.join('pages', pageName)
                commons.writeFile(dir, pageName + _extname, content);
            })
        })
    })
}
